...
// Par défaut, on affiche la vue 'home' (Home Assistant style), sinon le groupe demandé
...
// Variables pour la page d'accueil (Home Assistant)
$home_gas = null;
$home_g7b = null;
$home_imu = null;
$home_g7c = null;
...
try {
    ...
    elseif ($view_group === 'A') {
        ...
        // Récupération de la distance arrière pour votre vue combinée
        $stmt_b = $pdo->query("SELECT distance_cm FROM historique_capteur_g7b_recul ORDER BY date_evenement DESC LIMIT 1");
        $latest_g7b = $stmt_b->fetch();
        if ($latest_g7b) { $distance_arriere = $latest_g7b['distance_cm']; }
    } elseif ($view_group === 'E') {
...
<style>
    ...
    .radar-car .sensor-box { text-align: center; background: rgba(255,255,255,0.1); padding: 15px 25px; border-radius: 8px; }
...
<div class="container" style="max-width: 1280px; margin-top: 20px;">
    ...
    <?php if ($view_group === 'home'): ?>
        ...
        <div class="ha-grid">
            ...
            <div class="ha-card">
                ...
                <div>
                    <div class="ha-card-header"><span class="ha-icon">🚨</span> Télémétrie Arrière</div>
                    <div class="ha-state"><?= $home_g7b ? htmlspecialchars($home_g7b['distance_cm']) . ' cm' : '--' ?></div>
                </div>
            ...
            <div class="ha-card">
                ...
                <div>
                    <div class="ha-card-header"><span class="ha-icon">🧭</span> Obstacles Avant & GPS</div>
                    <div class="ha-state"><?= $home_g7c ? htmlspecialchars($home_g7c['distance_cm']) . ' cm' : '--' ?></div>
                </div>
                ...
                <?php if (count($home_logs) > 0): ?>
                    <?php foreach ($home_logs as $log): ?>
                        <div class="logbook-item">
                            <div>
                                <span style="color: var(--danger); font-weight: bold;">[ALERTE ROVER]</span> 
    ...
    <?php elseif ($view_group === 'A'): ?>
        ...
        foreach (array_reverse($mesures) as $m) {
            $chart_labels[] = date('H:i:s', strtotime($m['created_at']));
            $chart_data[] = $m['gas_value'];
        }
        ...
        <script>
            new Chart(document.getElementById('gasChart'), {
                type: 'line',
                data: { labels: <?= json_encode($chart_labels) ?>, datasets: [{ label: 'Concentration de Gaz (ppm)', data: <?= json_encode($chart_data) ?>, borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)', borderWidth: 2, fill: true, tension: 0.3 }] }
            });
        </script>
    ...
    <?php elseif ($view_group === 'B'): ?>
        ...
        foreach (array_reverse($mesures) as $m) {
            $chart_labels_b[] = date('H:i:s', strtotime($m['date_evenement']));
            $chart_data_b[] = floatval(str_replace(['>', '<'], '', $m['distance_cm']));
        }
    ...
    <?php elseif ($view_group === 'C'): ?>
        <h3 style="margin-top: 0;">Radar d'approche (Capteurs combinés)</h3>
        <div class="radar-car">
            <div class="sensor-box">
                <div style="font-size: 0.85em; color: #94a3b8; text-transform: uppercase;">Avant (Votre Capteur G7C)</div>
                <div style="font-size: 2.5em; font-weight: bold; color: #10b981;"><?= isset($mesures[0]) ? htmlspecialchars($mesures[0]['distance_cm']) : '--' ?> cm</div>
            </div>
            <div style="font-size: 5em; transform: rotate(270deg);">🚙</div>
            <div class="sensor-box">
                <div style="font-size: 0.85em; color: #94a3b8; text-transform: uppercase;">Arrière (Capteur G7B)</div>
                <div style="font-size: 2.5em; font-weight: bold; color: #ef4444;"><?= htmlspecialchars($distance_arriere) ?> <?= $distance_arriere !== '--' && !str_contains($distance_arriere, '>') ? 'cm' : '' ?></div>
            </div>
        </div>
        ...
        <script>
            ...
            function updateMapTracking(index) {
                const actualIndex = (rawData.length - 1) - index; const selectedRecord = rawData[actualIndex]; if (!selectedRecord) return;
